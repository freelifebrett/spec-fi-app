...
function mapFormDataToXML(formData, merchantData) {
    return {
        ...
        application: {
            ...
            busPhone: formData.phoneNumber.replace(/-/g, ''),
            celPhone: formData.phoneNumber.replace(/-/g, ''),
            ...
            CCAddress: '',
            CCIssuerName: '',
            CCCardHolderName: [formData.firstName, formData.middleName, formData.lastName].filter(name => name && name.trim()).join(' '),
